...
class CinderellaGirls:
    def count(self, talent, skill):
        girls = [(x, y) for x, y in zip(talent, skill)]
        girls.sort(key=lambda x: (x[0] - x[1] / 20000), reverse=True)
        max_skill = [0] * len(talent)
        max_skill[0] = girls[0][1]
        for i in range(1, len(talent)):
            max_skill[i] = max(max_skill[i-1], girls[i][1])

        count = 1
        for i in range(1, len(talent)):
            if max_skill[i-1] <= girls[i][1]:
                count += 1
