...
class BadNeighbors:
    def _helper(self, donations):
        max_donations = list(donations[:])
        for i in range(2, len(donations)):
            max_donations[i] += max(max_donations[i-2], (max_donations[i-3] if i > 2 else 0))
        return max(max_donations)

    def maxDonations(self, donations):
        return max(
            self._helper(donations[:-1]),
            self._helper(donations[1:])
        )
