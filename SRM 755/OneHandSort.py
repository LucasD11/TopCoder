...
class OneHandSort:
    ...
    def sortShelf(self, target):
        ...
        for i in range(n-1):
            if shelf[i] == final[i]:
                continue
            if shelf[i] is not None:
                self._exchange(shelf, i, shelf.index(None))
            if shelf[i] == final[i]:
                continue
            self._exchange(shelf, i, shelf.index(final[i]))
...
def run_tests():
    sys.stdout.write("OneHandSort (500 Points)\n\n")
